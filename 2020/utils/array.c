...
void array_resize(struct _array* arr, size_t n_newchunks) {
	...
	assert(newptr != NULL);
	arr->data = newptr;
	arr->n_allocatedchunks = n_newchunks;
...
size_t array_length(struct _array* arr) {
	assert(arr != NULL);

	return arr->n_usedchunks;
}
...
void array_append(struct _array* arr1, struct _array* arr2) {
	assert(arr1 != NULL);
	assert(arr2 != NULL);
	assert(arr1->chunksize == arr2->chunksize);

	size_t n_free1 = arr1->n_allocatedchunks - arr1->n_usedchunks;
	size_t n_used2 = arr2->n_usedchunks;

	ssize_t n_left1 = n_free1 - n_used2;

	if (n_left1 < 0) {
		size_t n_newchunks = arr1->n_allocatedchunks - n_left1;
		array_resize(arr1, n_newchunks);
	}

	size_t displacement = arr1->n_usedchunks * arr1->chunksize;
	size_t size2 = arr2->n_usedchunks * arr2->chunksize;
	memcpy(arr1->data + displacement, arr2->data, size2);
